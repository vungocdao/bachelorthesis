...
def round(ds):
    ...
    df.loc[(df['Polarity_result'] >= 0.5), 'Polarity_rounded'] = 1
    df.loc[(df['Polarity_result'] <= -0.5), 'Polarity_rounded'] = -1
    df.loc[(df['Polarity_result'] < 0.5) & (df['Polarity_result']> -0.5), 'Polarity_rounded'] = 0
    df['Polarity_rounded'] = df['Polarity_rounded'].astype(int)
    df.to_csv("Textblob_Analysed_Rounded_" + ds, index=False)
round(ds1)
round(ds2)
round(ds3)
round(ds4)
